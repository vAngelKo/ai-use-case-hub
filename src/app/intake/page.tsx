...
export default function IntakePage() {
  const [structuredForm, setStructuredForm] =
    useState<StructuredFormState>(EMPTY_STRUCTURED_FORM);
  ...
  const formComplete = useMemo(
    () => isStructuredFormComplete(structuredForm),
    [structuredForm]
  );
  ...
  const showGenerateButton =
    readyToGenerate && formComplete && view === "chat" && !useCaseData;

  const sendMessage = async () => {
    const text = input.trim();
    if (!text || isLoading || !formComplete) return;
    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          ...
          structuredFields: structuredForm,
        }),
      });
      ...
      if (reader) {
        ...
        if (fullContent.includes(READY_TOKEN)) {
          ...
          const finalDisplay = fullContent
            .replace(/\s*\[READY_FOR_OUTPUT\]\s*$/g, "")
            .trim();
          setMessages((prev) =>
            prev.map((m) =>
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `error-${Date.now()}`,
          role: "assistant",
          content: `Sorry, there was an error: ${errMsg}. Please check OPENAI_API_KEY and try again.`,
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setUseCaseData(json as UseCaseOutput);
      setView("review");
    } catch (e) {
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="text-xs text-slate-500 mb-4">
              <span className="font-medium text-slate-600">ROI details</span>{" "}
              and{" "}
              <span className="font-medium text-slate-600">hours saved</span>{" "}
              reflect what you described in chat—you can edit them below. The
              standardized{" "}
              <span className="font-medium text-slate-600">value type</span>{" "}
              and{" "}
              <span className="font-medium text-slate-600">
                escalation tier
              </span>{" "}
              are included when you submit.
            </p>
            <ReviewForm data={useCaseData} onChange={setUseCaseData} />
          </div>

          <SubmitRequestButton data={useCaseData} />
        </div>
      </div>
    );
  }
      ...
      <header className="border-b border-slate-200 pb-4 mb-4">
        <div className="flex justify-between items-start gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Pick standardized options below, then chat to describe your use
              case.
            </p>
          </div>
          <Link
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={sendMessage}
            disabled={isLoading || !input.trim() || !formComplete}
